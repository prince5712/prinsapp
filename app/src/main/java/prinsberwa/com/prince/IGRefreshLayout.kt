...
import prinsberwa.com.prince.progressBar.BaseProgressBar
import prinsberwa.com.prince.progressBar.CircleProgressBar
...
class IGRefreshLayout @JvmOverloads constructor(
    ...
    init {
        ...
        // Replace the hardcoded color with theme attribute lookup
        val typedValue = TypedValue()
        if (context.theme.resolveAttribute(R.attr.refreshLayoutBackground, typedValue, true)) {
            setBackgroundColor(ContextCompat.getColor(context, typedValue.resourceId))
        } else {
            // Fallback color if attribute not found
            setBackgroundColor(Color.DKGRAY) // Keep as fallback
        }
            ...
            for (i in 0 until childCount) {
                ...
                if (child != mBar) {
                    ...
                    if (mTarget?.background == null) {
                        ...
                        if (theme.resolveAttribute(R.attr.toolbar_bg, typedValue, true)) {
                            mTarget?.setBackgroundColor(ContextCompat.getColor(context, typedValue.resourceId))
                        }
                    }
